...
<script>
  ...
  const externalHeaders = {
    'X-LTAI-EXT-AUTH-TOKEN': 'your-access-token',
    'X-LTAI-EXT-API-TOKEN': 'your-agent-api-token',
...
```ts
{
  'X-LTAI-EXT-AUTH-TOKEN': '<access token>',
...
<script>
  ...
  if (accessToken) derivedHeaders['X-LTAI-EXT-AUTH-TOKEN'] = accessToken;
